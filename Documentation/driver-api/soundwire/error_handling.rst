...
2. ClockStop: If one Slave is not capable of engaging the clock stop and the
   system still needs to suspend, the priority might be to suspend anyway and
   reset the bus on startup. This would prevent any active standby/always-on
   activity but would not impact power targets.
