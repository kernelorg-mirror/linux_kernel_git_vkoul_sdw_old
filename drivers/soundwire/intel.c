...
intel_pdi_shim_configure(struct sdw_intel *sdw, struct sdw_cdns_pdi *pdi)
{
	...
	pdi->link_id = (link_id * 16) + pdi->num + 5;
	...
	pdi_conf |= (pdi->link_id << SDW_REG_SHIFT(SDW_SHIM_PCMSYCM_STREAM));
...
intel_pdi_alh_configure(struct sdw_intel *sdw, struct sdw_cdns_pdi *pdi)
{
	...
	conf = intel_readl(alh, SDW_ALH_STRMZCFG(pdi->link_id));
	...
	intel_writel(alh, SDW_ALH_STRMZCFG(pdi->link_id), conf);
}
