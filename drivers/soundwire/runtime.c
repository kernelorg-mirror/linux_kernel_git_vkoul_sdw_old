			...
			struct sdw_port_runtime *p_rt)
{
	...
	dpn_prop = sdw_get_slv_dpn_prop(s_rt->slave,
					s_rt->direction,
					t_params->port_num);
	if (!dpn_prop)
		return -EINVAL;
	...
	if (ret < 0) {
		dev_err(bus->dev, "DPN_PortCtrl register write failed");
		goto error;
	}
	...
	if (ret < 0) {
		dev_err(bus->dev, "DPN_BlockCtrl1 register write failed");
		goto error;
	}
	...
	if (ret < 0) {
		dev_err(bus->dev, "DPN_SampleCtrl1 register write failed");
		goto error;
	}
	...
	if (ret < 0) {
		dev_err(bus->dev, "DPN_OffsetCtrl1 register write failed");
		goto error;
	}
	...
	if (t_params->blk_grp_ctrl_valid) {
		...
		if (ret < 0) {
			dev_err(bus->dev, "DPN_BlockCtrl2 reg write failed");
			goto error;
		}
	}
	...
	if (slave_prop->lane_control_support) {
		...
		if (ret < 0) {
			dev_err(bus->dev, "DPN_LaneCtrl register write failed");
			goto error;
		}
	}
...
error:
	dev_err(&s_rt->slave->dev, "Transport reg write failed for port: %d",
						t_params->port_num);
	return ret;
...
static int sdw_program_port_params(struct sdw_master_runtime *m_rt)
{
	...
	list_for_each_entry(p_rt, &m_rt->port_list, port_node) {
		...
		/* Program transport & port parameters for Master */
				...
				struct sdw_port_runtime *p_rt, bool en)
{
	...
	if (ret < 0)
		dev_err(bus->dev, "Slave chn_en reg write failed");
			...
			struct sdw_port_runtime *p_rt, bool en)
{
	...
	enable_ch.enable = en; /* Enable/Disable */
...
/**
 * sdw_enable_disable_ports: Enable/disable port(s) for Master(s) and
 * Slave(s)
 *
 * @m_rt: Runtime Master handle
...
static int sdw_enable_disable_ports(struct sdw_master_runtime *m_rt, bool en)
{
	...
	list_for_each_entry(s_rt, &m_rt->slave_list, master_node) {
		list_for_each_entry(s_port, &s_rt->port_list, port_node) {
			ret = sdw_enable_disable_slv_ports(m_rt->bus, s_rt,
							s_port, en);
			if (ret < 0)
				return ret;
		}
			...
			struct sdw_port_runtime *p_rt, bool prep)
{
	...
	if (!dpn_prop->simple_ch_prep_sm) {
		...
		if (ret < 0) {
			dev_err(bus->dev, "Slave prep_ctrl reg write failed");
			goto error;
		}
				...
				struct sdw_port_runtime *p_rt, bool prep)
{
	...
	/* Post-prepare/Post-deprepare port(s) */
	if (ops->dpn_port_prep) {
...
static int sdw_prep_deprep_ports(struct sdw_master_runtime *m_rt, bool prep)
{
	...
	list_for_each_entry(s_rt, &m_rt->slave_list, master_node) {
		list_for_each_entry(p_rt, &s_rt->port_list, port_node) {
			ret = sdw_prep_deprep_slv_ports(m_rt->bus, s_rt,
							p_rt, prep);
			if (ret < 0)
				return ret;
		}
...
static int sdw_program_params(struct sdw_bus *bus)
{
	...
	list_for_each_entry(m_rt, &bus->rt_list, bus_node) {
...
/**
 * sdw_deprepare_stream: De-prepare stream
 *
...
/**
 * sdw_disable_stream: Disable stream
 *
 * @stream: Stream runtime handle
 */
...
/**
 * sdw_enable_stream: Enable stream
 *
 * @stream: Stream runtime handle
 */
