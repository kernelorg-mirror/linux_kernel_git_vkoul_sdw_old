...
static int rt700_set_tdm_slot(struct snd_soc_dai *dai, unsigned int tx_mask,
				     ...
				     struct snd_soc_dai *dai)
{
	...
	struct sdw_ports_config port_config;
	enum sdw_data_direction direction;
	struct sdw_port_config port_cfg;
	struct sdw_stream_data *stream;
	...
	retval = sdw_config_stream(rt700->slave->bus,
			rt700->slave, &stream_config, stream->tag);
	if (retval) {
	...
	port_config.num = 1;
	port_config.port_config = &port_cfg;
	port_cfg.num = port;
	num_channels = params_channels(params);
	port_cfg.ch_mask = (1 << (num_channels))  - 1;

	retval = sdw_config_ports(rt700->slave->bus, rt700->slave,
					&port_config, stream->tag);
	if (retval) {
				...
				struct snd_soc_dai *dai)
{
	...
	sdw_release_ports(rt700->slave->bus, rt700->slave, stream->tag);
	sdw_release_stream(rt700->slave->bus, rt700->slave, stream->tag);
	return 0;
}
...
static struct snd_soc_dai_ops rt700_ops = {
	...
	.set_tdm_slot	= rt700_set_tdm_slot,
